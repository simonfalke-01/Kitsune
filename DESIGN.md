...
- Without a selection, the challenge collection is the primary focus. The
  detail pane remains blank until the competitor chooses an object to inspect.
- The event trail keeps event, progress, rank, and live score context stable.
  The unselected view does not repeat those values in a secondary dashboard.
...
- On ordinary unsolved rows, trusted author metadata replaces the redundant
  `Unsolved` line as muted `by <name>` text beneath the challenge name. Solved
...
- Automatic teammate presence may replace ordinary unsolved metadata while a
  teammate is actively viewing that challenge. It remains inside the invariant
  second line, names the teammate in text, expires without manual cleanup, and
  is never visible outside the current team. Solved state remains visible when
  presence is added beside it.

### 8.3 Split workspace

- The document and workspace root never scroll. The collection is always a
  desktop scroll owner; the right-pane body becomes the second only when a
...
- Realtime interruption preserves the last trustworthy challenge state. The
  event trail reports only actionable connection loss, reconnects with bounded
  backoff, and reconciles challenges after recovery without clearing stable
  content.

### 8.4 Unselected challenge view

- Keep the established desktop split and collection width. Do not expand the
  challenge list across the workspace merely because no challenge is selected.
- Center one quiet `No challenge selected` prompt in the otherwise empty detail
  pane. Do not add instructions, event metrics, team analytics, a challenge-field
  summary, illustration, action, border, or decorative empty-state content.
- Selecting a challenge fills that stable pane without moving or remounting the
  collection.
- Preserve the existing team-performance composition and supporting chart,
  standings, and category-progress components in source for a future team route.
  That route is not yet designed, so the preserved view does not ship here.

### 8.5 Challenge detail

- Header contains name, points, minimal state, and decision-relevant metadata.
- The header exposes one quiet copy-link action using the permission-scoped
  challenge URL already represented by workspace state.
...
- The description may use one bounded region because it is a discrete authored
  object. Authored Markdown renders as safe semantic headings, prose, lists,
  links, quotes, tables, and code without interpreting raw HTML. Nested
  sub-sections rely on whitespace before borders.
...
- The current team remains at its true ordinal position. As soon as that row
  crosses a solve-viewport edge, a non-semantic visual dock follows its
  direction: top when the row crosses above and bottom when it crosses below.
...
- First-three-plus-self context supports the Details solving flow, entering
  ...
  in the leading column and 3rd then self in the trailing column when space is
  tight, then returns to one line when the detail pane can hold all four
  identities. Its subparts are connected by alignment, not mini cards.
